...
class Light:
    def __init__(self, light_type, color, position_or_direction):
        self.type = light_type  # 'point' or 'directional'
        self.color = color
...
def read_scene_file(scene_file):
    scene = None
    lights = []  # Initialize an empty list to store lights
    with open(scene_file, 'r') as f:
        for line in f:
            ...
            # Check for the light section in the scene file
            elif tokens[0] == 'light':
                light_type = tokens[1]
                color = list(map(int, tokens[2:5]))
                position_or_direction = list(map(float, tokens[5:]))
                lights.append(Light(light_type, color, position_or_direction))
...
def render_pixel(i, j, scene):
    ...
    if closest_intersection is not None:
        return (i, j, phong_shading(closest_intersection.object.material, scene.lights[0], closest_intersection))
...
def phong_shading(material, lights, intersection):
    ...
    if lights:  # Check if lights list is not empty
        light = lights[0]  # Access the first light (you might modify this based on your lighting logic)
        ...
        diffuse_intensity = max(0, np.dot(intersection.object.normal, light_direction))
        diffuse = material.diffuse_color * light.color * diffuse_intensity

        # Calculate specular component
        reflection = 2 * np.dot(intersection.object.normal, light_direction) * intersection.object.normal - light_direction
        specular_intensity = max(0, np.dot(reflection, -np.array(intersection.point)))
        specular = material.specular_color * light.color * (specular_intensity ** material.shininess)

        # Calculate final color using ambient, diffuse, and specular components
        ...
        # Handle the case when there are no lights in the scene
        print("No lights in the scene!")
        return [0, 0, 0]  # Returning default color (black) as there are no lights
...
if __name__ == "__main__":
    ...
    # Store the original lights from the parsed scene
    ...
    while selected_input != "exit":
        main_scene.lights = original_lights  # Restore the original lights before rendering
        ...
        # Modify the scene to include lights and materials
        main_scene.lights = [
            Light(light_type='point', color=[255, 255, 255], position_or_direction=[5, 5, 10]),
            Light(light_type='directional', color=[255, 255, 255], position_or_direction=[-1, -1, -1])
        ]

        main_scene.objects = [
            Sphere(center=[2, 0, 0], radius=2,
                   material=Material(diffuse_color=[255, 0, 0], specular_color=[255, 255, 255], shininess=32)),
            # ... (Add other objects with materials)
        ]
